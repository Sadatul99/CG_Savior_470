...
async function run() {
  try {
    ...
    app.get('/courses/:code', async (req, res) => {
      ...
      try {
        const course = await courseCollection.findOne({ course_code: code });
    ...
    app.patch('/courses/:code', async (req, res) => {
      const code = req.params.code.toUpperCase();
      const data = req.body;
      const updatedCourse = {
        $set: {
      ...
      try {
        const course = await courseCollection.updateOne({ course_code: code }, updatedCourse);
        if (!course) return res.status(404).send({ message: 'Course not found' });
        res.json(course);
      } catch (err) {
        res.status(500).send({ message: err.message });
